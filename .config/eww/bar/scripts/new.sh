...
print_workspaces() {
    ...
    for ((i = 0; i < ${#desktops[@]}; i++)); do
        ...
        buf="$buf (eventbox :cursor \"hand\" (button :class \"workspaces-button $class\" :onclick \"xdotool key super+$((i+1))\" \"$icon\"))"
    done

    echo "(box :class \"workspaces\" :halign \"center\" :valign \"center\" :vexpand \"false\" :hexpand \"false\" $buf)"
}
