...
is_wifi () {
  ...
  if [[ "$iswifi" = "connected" ]]; then
    echo "(button :class \"wifis\" :onclick \"eww open wifi-wid --config /home/termai/.config/eww/bar_dwm2/widgets/wifi-wid\" :tooltip \"$is_wifis\" \"󰖩\" )"
  else
    echo "󰖪"
  fi
}
...
is_power() {
  eww open powerDash --config /home/termai/.config/eww/bar_dwm2/power_dash/.
}
