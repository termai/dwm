...
wait_media() {
  ...
  if [ -e "$svg_media" ]; then
    one="(image :class \"media-image\" :path \"$svg_media\" :image-width 34 image-height 34)"
  elif [ -e "$png_media" ]; then
    one="(image :class \"media-image\" :path \"$png_media\" :image-width 34 image-height 34)"
  else
    one=""
  fi
  ...
  coverart="(image :class \"cover-art\" :path \"/tmp/cover.jpg\" :image-width 38 image-height 34)" 
...
is_wifi () {
  ...
  if [[ "$iswifi" = "connected" ]]; then
    echo "(button :class \"wifis\" :onclick \"eww open wifi-wid --config /home/termai/.config/eww/bar_xmonad/widgets/wifi-wid\" :tooltip \"$is_wifis\" \"󰖩\" )"
  else
    echo "󰖪"
  fi
}
