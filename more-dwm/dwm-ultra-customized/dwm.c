...
void setfullscreen(Client *c, int fullscreen) {
    if (fullscreen && !c->isfullscreen) {
        // Save client's current dimensions and position
        c->oldx = c->x;
        c->oldy = c->y;
        c->oldw = c->w;
        c->oldh = c->h;
        c->oldbw = c->bw;
        // Remove borders and make client floating
        c->bw = 0;
        c->isfloating = 1;
        // Set client to full screen
        c->x = c->mon->mx + c->mon->gappx;
        c->y = c->mon->my + c->mon->gappx;
        c->w = c->mon->mw - 2 * c->mon->gappx;
        c->h = c->mon->mh - 2 * c->mon->gappx;
        c->isfullscreen = 1;
        // Raise client window to top of the stack
        XRaiseWindow(dpy, c->win);
    } else if (!fullscreen && c->isfullscreen) {
        // Restore client's previous dimensions and position
        c->x = c->oldx;
        c->y = c->oldy;
        c->w = c->oldw;
        c->h = c->oldh;
    ...
    // Resize client with gaps
    if (c->isfullscreen) {
        resizeclient(c, c->mon->mx + c->mon->gappx, c->mon->my + c->mon->gappx,
                     c->mon->mw - 2 * c->mon->gappx, c->mon->mh - 2 * c->mon->gappx);
    } else {
...
tile(Monitor *m)
{
	...
	if (n == 1 && selmon->sel->CenterThisWindow)
        ...
        //selmon->mw/2 - selmon->mw * 0.45,
        //selmon->mh/2 - selmon->sel->h * 0.45,
        //selmon->mw * 0.9,
        //selmon->sel->h * 0.9);

        selmon->mw/2 - selmon->mw * 0.475,
        selmon->mh/2 - selmon->sel->h * 0.45,
        selmon->mw * 0.95,
        selmon->sel->h * 0.9);
}
