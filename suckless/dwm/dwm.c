...
drawbar(Monitor *m)
{
  ...
  unsigned int i, occ = 0, urg = 0;
    ...
    //drw_text(drw, x, vertpadbar / 2, w, bh - vertpadbar, 0, stext, 0);
	}
...
monocle(Monitor *m)
{
    ...
    resize(c, m->wx + gappoh, m->wy + gappov, m->ww - 2 * gappoh, m->wh - 2 * gappov, 0);
}*/
...
monocle(Monitor *m)
{
  ...
  /*if (n > 0) // override layout symbol 
  {
    n = 0; // Reset n to 0 before counting visible tiled clients again
    for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
        n++;
  ...
  for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
    int newx = m->wx - c->bw + m->gappoh * oe;
    int newy = m->wy - c->bw + m->gappov * oe;
    int neww = m->ww - 2 * (m->gappoh * oe + c->bw);
    int newh = m->wh - 2 * (m->gappov * oe + c->bw);
...
monocle(Monitor *m)
{
  ...
  for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
    ...
    if (m->gappoh == 0) {
      ...
      newx = m->wx + m->gappov*oe - c->bw;
      newy = m->wy + m->gappoh*oe - c->bw;
      neww = m->ww - 2 * (m->gappoh*oe + c->bw);
      newh = m->wh - 2 * (m->gappov*oe + c->bw);
    }
    ...
    //resize(c, newx, newy, neww, newh, 0);
    resize(c, newx - c->bw, newy - c->bw, neww + 2 * c->bw, newh + 2 * c->bw, 0);
