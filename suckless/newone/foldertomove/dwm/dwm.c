...
drawbar(Monitor *m)
{
  ...
  if ((w = m->ww - tw - x) > bh) {
    if (m->sel) {
        ...
        // Calculate the width of the client name
        int text_width = drw_fontset_getwidth(drw, m->sel->name);

        // Calculate the starting position for centering the text
        x = x + (w - text_width) / 2;

        drw_text(drw, x, 0, text_width - 2 * sp, bh, lrpad / 2, m->sel->name, 0);
...
loadxrdb()
{
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        XRDB_LOAD_COLOR("dwm.color0", normalbordercolor);
        XRDB_LOAD_COLOR("dwm.color0", normalbgcolor);
        XRDB_LOAD_COLOR("dwm.color4", normalfgcolor);
        XRDB_LOAD_COLOR("dwm.color8", selcbordercolor);
        XRDB_LOAD_COLOR("dwm.color4", selcbgcolor);
        XRDB_LOAD_COLOR("dwm.color0", selcfgcolor);
      }
    }
  }
